...
cv::Mat rkYolov11::infer(cv::Mat &orig_img) {
    ...
    // Debug print the image info
    debug_printf("Source image: width=%d, height=%d, size=%d, channels=%d\n",
           img.cols, img.rows, img.cols * img.rows * 3, img.channels());
    debug_printf("Model expects: width=%d, height=%d, channels=%d, total size=%d\n",
    ...
    // Run inference with debug print
    debug_printf("Running inference with image_buffer: width=%d, height=%d, format=%d, size=%d\n",
